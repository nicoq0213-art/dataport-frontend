...
function generarAnual(data) {
  ...
  // 1. Mercaderías
  if (merc.total != null) {
    let p = `Durante el período analizado, el puerto movilizó un total de ${fmtTn(merc.total)}, registrando ${varFrase(merc.var_pct)}.`;
    if (merc.importacion != null && merc.exportacion != null) {
      p += ` El flujo de importación representó ${fmtTn(merc.importacion)}, mientras que la exportación alcanzó ${fmtTn(merc.exportacion)}`;
      if (merc.removido != null && merc.removido > 0) {
        p += ` y las operaciones de removido sumaron ${fmtTn(merc.removido)}`;
      }
      p += ".";
    }
    parrafos.push(p);
  ...
  if (cont.teus != null) {
    ...
    if (cont.var_pct_teus != null) {
      p += `, lo que representa ${varFrase(cont.var_pct_teus)} en términos de TEUs`;
    }
    p += ".";
    parrafos.push(p);
  }
  ...
  if (nav.total_buques != null) {
    ...
    if (nav.total_trn != null) {
      p += `, con un total de ${fmtNum(nav.total_trn)} TRN (toneladas de registro neto)`;
    }
  ...
  // 4. Comparativo mercaderías
  if (totales.mercaderias?.anterior != null && totales.mercaderias?.actual != null) {
  ...
  if (perm.ranking_anual?.length > 0) {
    ...
    if (topEmp) {
      p += `, destacándose ${topEmp} como las principales en términos de volumen operado`;
    }
    p += ".";
    parrafos.push(p);
  }
...
function generarMensual(data) {
  ...
  return porMes.map(m => {
    ...
    // Recalcular variación (applyFilters puede haber modificado merc_act sin actualizar merc_var)
    const varMerc = merAnt > 0 ? ((merAct - merAnt) / merAnt * 100) : 0;

    const teusAnt = safe(m.teus_ant);
    const teusAct = safe(m.teus_act);
    const varTeus = teusAnt > 0 ? ((teusAct - teusAnt) / teusAnt * 100) : safe(m.teus_var);

    const bqAnt = safe(m.buques_ant);
    const bqAct = safe(m.buques_act);
    const varBq  = bqAnt > 0 ? ((bqAct - bqAnt) / bqAnt * 100) : safe(m.buques_var);

    const c = cargasPorMes[m.mes] || {};

    let texto = `En ${m.mes}, el puerto registró un movimiento de ${fmtTn(merAct)}, `;
    texto += varMerc !== 0
      ? `evidenciando ${varFraseMes(varMerc)}. `
      : `sin variación significativa respecto al mismo mes del año anterior. `;

    // Desglose imp/exp/rem si disponible
    if (c.importacion != null || c.exportacion != null) {
      const partes = [];
      if (c.importacion != null) partes.push(`importación: ${fmtTn(c.importacion)}`);
      if (c.exportacion != null) partes.push(`exportación: ${fmtTn(c.exportacion)}`);
      if (c.removido    != null && c.removido > 0) partes.push(`removido: ${fmtTn(c.removido)}`);
      if (partes.length > 0) texto += `El desglose por operación fue: ${partes.join(", ")}. `;
    }

    // TEUs
    if (teusAct > 0) {
      texto += `Se movilizaron ${fmtNum(teusAct)} TEUs`;
      if (Math.abs(varTeus) >= 0.1) {
    ...
    if (bqAct > 0) {
      ...
      if (Math.abs(varBq) >= 0.1) {
        texto += `, representando ${varFraseMes(varBq)}`;
      }
      texto += `.`;
    }

    return { mes: m.mes, texto };
  });
}
...
export default function Informe({ data, filtros = {} }) {
  ...
  const parrafosAnual   = generarAnual(data);
  const registrosMensual = generarMensual(data);
